...
describe("Create Quiz Modal", function () {
  ...
  it("Check add new quiz button", async () => {
    const addNewQuizBtn = await page.evaluate(
      () => document.querySelector(".add-quiz-link").textContent
    );
  ...
  it("Check successfully create quiz", async () => {
    ...
    const modalTitle = await page.evaluate(
      () => document.querySelector(".create-quiz-modal h2").textContent
    );
    ...
    await page.evaluate(() =>
      document.querySelector(".create-quiz-btn").click()
    );
