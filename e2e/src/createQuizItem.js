...
describe("Create Quiz Item Test", function () {
  ...
  it("Should show add quiz item page", async function () {
    ...
    await page.goto(
      "http://localhost:3000/quizitem/quiz/" + quizName + "/add-quiz-item"
    );
    ...
    const quizText = await page.evaluate(
      () => document.querySelector(".quiz-name").textContent
    );
    // Make sure the page has title
  ...
  it("Check page elements loaded correctly", async () => {
    ...
    const optionFields = await page.evaluate(
      () => document.querySelectorAll(".option-text").length
    );
    // Make sure the page has title
    assert.equal(optionFields, 4, "All Option Field Show");

    const optionCheckbox = await page.evaluate(
      () => document.querySelectorAll(".answer-checkbox").length
    );
    // Make sure the page has title
    ...
    await page.evaluate(() =>
      document.querySelector(".create-quiz-item-btn").click()
    );
  ...
  it("Check quiz item is added correctly", async () => {
    ...
    assert.equal(
      ...
      "Option 4",
      "Correct answer is set correctly"
    );
    assert.equal(
      quizItemDetails.question,
      "Test Quiz Item",
      "Correct qustion is set correctly"
    );
