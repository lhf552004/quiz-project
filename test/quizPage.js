...
describe("Single Quiz Page", function () {
	...
	it("Should has next button", async () => {
		...
		const buttonText = await page.evaluate(() => document.querySelector('.next-button').textContent);
	...
	it("Check right answer step", async () => {
		...
		.then(async (resp) => {			
			...
			await page.waitForSelector('.quiz-answer[data-ans-value="' + quizItemDetails.answer + '"]', {visible: true});
			await page.waitForSelector('button.btn.btn-primary.next-button', { visible: true });	

			await page.evaluate(async (quizItemDetails) => {
				// Trigger click on right answer and next button
				document.querySelector('.quiz-answer[data-ans-value="' + quizItemDetails.answer + '"]').click();
			...
			const hasRightClass = await page.evaluate((quizItemDetails) => document.querySelector('.quiz-answer[data-ans-value="' + quizItemDetails.answer + '"]').classList.contains('right'), quizItemDetails);
			...
			const nextQuizItem = await page.evaluate(() => document.querySelector('.quiz-item.active').dataset.id);
	...
	it("Check wrong answer step", async () => {
		...
		.then(async (resp) => {			
			...
			await page.waitForSelector('.quiz-item.active .quiz-answer[data-ans-value]:not([data-ans-value="' + quizItemDetails.answer + '"])', {visible: true});
			await page.waitForSelector('button.btn.btn-primary.next-button', { visible: true });	

			await page.evaluate(async (quizItemDetails) => {
				document.querySelector('.quiz-item.active .quiz-answer[data-ans-value]:not([data-ans-value="' + quizItemDetails.answer + '"])').click();
				document.querySelector('button.btn.btn-primary.next-button').click();
			...
			const hasWrongClass = await page.evaluate(() => document.querySelector('.quiz-item.active .quiz-answer.active').classList.contains('wrong'));
	...
	it("Check result", async() => {		
		...
		const numberOfQues = await page.evaluate(() => document.querySelectorAll('.quiz-item').length);
		...
		const numberRight = await page.evaluate(() => document.querySelectorAll('.quiz-item').length - document.querySelectorAll('.quiz-answer.wrong').length);

		// make sure result is visible
		await page.waitForSelector('.finish-quiz', {visible: true});

		// get result text
		const resultText = await page.evaluate(() => document.querySelector('.finish-quiz h4').textContent);

		// expected text just based on the number of right answer and number of question
		const expectResultText = 'You got ' + numberRight + '/' + numberOfQues + ' right';
